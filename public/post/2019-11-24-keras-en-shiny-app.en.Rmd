...
Como ya se mencionó, en el `server` van las instrucciones necesarias para obtener los resultados, en este caso, se debe tomar la imagen desde el input (`fileInput()`) para luego procesarla y realizar la predicción del modelo (`renderPrint()` porque en el `ui` la salida quedó definida con `verbatimTextOutput()` con id = `resultados` ):  
